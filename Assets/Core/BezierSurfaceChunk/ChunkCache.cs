...
public class ChunkCache
{
    ...
    public void UpdateChunksToGenerate(int genDistance)
    {
        for (int x = ActiveChunk.x - genDistance; x <= ActiveChunk.x + genDistance; x++)
        {
            ...
            {                
                if (!GeneratedChunksMap.ContainsKey(new Vector2i(x, z)) && !ChunksInGeneration.Contains(new Vector2i(x, z)) && !ChunksToGenerate.Contains(new Vector2i(x, z)))
                {
        ...
        for (int i = 0; i < ChunksToGenerate.Count; i++)
        {
            ...
            Thread t = new Thread(() => GenerateChunk(ChunksToGenerate[i]));
            Debug.Log("Thread " + t.ToString() + " created");
            t.Start();
